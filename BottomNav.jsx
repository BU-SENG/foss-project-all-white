    ...
    <div className="fixed bottom-0 left-0 right-0 bg-[#0B1410] border-t border-[#13231B] px-6 py-3 z-50">
      <div className="max-w-md mx-auto flex justify-between items-center">
        {tabs.map((tab) => (
          ...
          >
            {tab.highlight ? (
              <div className="bg-[#00E359] p-3 rounded-full -mt-10 shadow-lg border-4 border-[#0B1410]">
                <tab.icon size={24} color="#000" />
                <span className="text-[10px] font-bold text-black mt-1 block">{tab.name}</span>
              </div>
            ) : (
              <>
                <tab.icon size={24} color={activeTab === tab.id ? '#00E359' : '#8E9B93'} />
                <span className={`text-[10px] mt-1 ${activeTab === tab.id ? 'text-[#00E359]' : 'text-[#8E9B93]'}`}>
                  {tab.name}
                </span>
              </>
            )}
